...
class MatrixFunctional(LinearSystemObservable):
    ...
    def _single_qubit_projectors(self):
    ...
    def observable(self, num_qubits: int) -> List[Operator]:
        ...
        """
        zero_op, one_op = self._single_qubit_projectors()
        I = Operator(Pauli('I')).data

        observables = []
        # Identity on all qubits for norm measurement
        identity_op = reduce(np.kron, [I] * num_qubits)
        observables.append(Operator(identity_op))

        for i in range(num_qubits):
            j = num_qubits - i - 1

            # Build operators as tensor products of I, zero_op, one_op
            def tensor_ops(ops):
                return reduce(np.kron, ops)

            if i > 0:
                op1 = tensor_ops(
                    [I] * j + [zero_op] + [one_op] * i
                )
                op2 = tensor_ops(
                    [I] * j + [one_op] + [one_op] * i
                )
                observables.append(Operator(op1))
                observables.append(Operator(op2))
            else:
                op1 = tensor_ops([I] * j + [zero_op])
                op2 = tensor_ops([I] * j + [one_op])
                observables.append(Operator(op1))
                observables.append(Operator(op2))
