...
class AbsoluteAverage(LinearSystemObservable):
    ...
    def observable(self, num_qubits: int)  -> Operator:
        ...
        """
        # Define single-qubit projector |0><0| = (I + Z)/2
        I = Operator(Pauli('I')).data
        Z = Operator(Pauli('Z')).data
        zero_op = (I + Z) / 2

        # Tensor product of zero_op repeated num_qubits times
        full_op = reduce(np.kron, [zero_op] * num_qubits)
