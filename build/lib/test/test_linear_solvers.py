...
class TestObservables(unittest.TestCase):
    ...
    def test_absolute_average(self, observable, vector):
        ...
        # Prepare the quantum circuit with the initial state
        qc = QuantumCircuit(num_qubits)
        qc.isometry(init_state, list(range(num_qubits)), None)

        # Append the observable circuit
        qc.append(observable.observable_circuit(num_qubits), list(range(num_qubits)))

        # Use AerSimulator to get the final statevector after applying the circuit
        simulator = AerSimulator(method='statevector')
        result = simulator.run(qc).result()
        final_state = result.get_statevector(qc)

        # Convert observable operator to Operator object
        observable_op = Operator(observable.observable(num_qubits))

        # Compute expectation value <psi|O|psi>
        expectation_value = np.vdot(final_state, observable_op.data @ final_state)

        # Post-process the result as per observable's method
        processed_result = observable.post_processing(expectation_value, num_qubits)

        # Compute the exact classical evaluation
        exact = observable.evaluate_classically(init_state)

        # Assert that the processed result matches the exact value within tolerance
        np.testing.assert_almost_equal(processed_result, exact, decimal=2)
    ...
    def test_matrix_functional(self, observable, vector):
        ...
        for obs_circ in obs_circuits:
            ...
            qc = qc.decompose(reps=10)  # Decompose Initialize to basic gates to avoid unsupported instructions
            qc.barrier()
            qc.append(obs_circ, qc.qubits)
        ...
        # Evaluate expectation values by simulating circuits and applying operators
        # First is the norm
        result = simulator.run(qcs).result()
        state_vecs = []
        for i, op in enumerate(observable_ops):
            statevector = result.get_statevector(i)
            state_vecs.append(np.vdot(statevector, op.data @ statevector))

        # For the rest, evaluate pairs
        for i in range(1, len(observable_ops), 2):
            for j in [i, i + 1]:
                result = simulator.run(qcs[j]).result()
                statevector = result.get_statevector(0)  # Always 0 because running one circuit at a time
                op = observable_ops[j]
                state_vecs.append(np.vdot(statevector, op.data @ statevector))

        # Obtain result via post-processing
...
class TestLinearSolver(unittest.TestCase):
    ...
    def test_hhl_sampler(self):
        """Test the HHL sampler getter and setter."""
        # Initialize a sampler (default options)
        sampler = Sampler()

        # Initialize HHL with the sampler
        hhl = HHL(sampler=sampler)
        self.assertEqual(hhl.sampler, sampler)  # Should be set correctly

        # Create a new sampler with different options
        new_sampler = Sampler(options={"shots": 1024})
        hhl.sampler = new_sampler
        self.assertEqual(hhl.sampler, new_sampler)

        # Setting sampler to None is not supported; test that it raises an error
        try:
            hhl.sampler = None
        except Exception as e:
            self.assertTrue(isinstance(e, (TypeError, ValueError)))
